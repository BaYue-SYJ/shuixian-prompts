#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把全部提示词重标为【单标签 17 类】（人物优先）。
category 字段写回「字符串」。
17 类：
  人物(2): 真人/写实人物, 动漫/二次元人物
  非人(15): 字体/排版/标题, Logo/品牌/VI, UI/App/网页/SaaS, 产品/电商/包装(含食物),
            海报/广告/社媒, 插画/艺术/概念, 漫画/分镜/故事板, 信息图/教育图解,
            3D/游戏/像素/等距, 建筑/室内/空间, 风景/自然, 动物/宠物, 车辆/机械/科幻,
            抽象/纹理/背景, 其他/未归类
判定：
  1) 命中人物词 -> 动漫/二次元(有动漫信号) 否则 真人/写实(默认)。
  2) 未命中人物 -> 非人关键词优先级，最高分类；全不中 -> 其他/未归类。
本地+部署同步改写；并再生 categories.json。
"""
import json, re, os, collections, glob, sys
...
# 17 类展示顺序（本地+部署 categories.json 用）
CATEGORIES_17 = [
    "真人/写实人物","动漫/二次元人物",
...
# ---------- 人物词（复用 reclassify_single 的成熟集合，足够覆盖"带人的"） ----------
...
# ---------- 动漫 vs 真人 信号 ----------
ANIME_KW = ["动漫","二次元","动画","漫画","卡通","日系插画","赛璐璐"," anime","anime ","anime,","manga","cartoon","cel shading","吉卜力","宫崎骏","日漫","comic style","2d","二次元风格","插画风","日式动","acg","galgame","立绘","二次元少女","动漫风","动画风","anime style","manga style","2d illustration","chibi","q版","二次元插画","comic","anime"]
REAL_KW  = ["真人","摄影","写实","照片","胶片","棚拍","人像摄影","realistic","real photo","photoreal","photograph","电影感","live action","real life","portrait photo","单反","影棚","光圈","写真摄影","纪实","街拍","时尚摄影","棚拍","real person","cosplay 真人","电影截图","实拍","人像写真","影棚写真","婚纱摄影","汉服摄影","和服"]
