...
const App = (() => {
  ...
  // ---------- 数据 ----------
  // 兼容两种数据源：部署版用拆分文件 prompts.part1/2/3.json，本地版用单文件 prompts.json
  ...
  function addFav(d, boardId){ favData.items[d.id] = { boardId: boardId || 'default', item:d }; saveFav(); }
  ...
  function makeCard(d, extraTags){
    ...
    const copyBtn = document.createElement('button'); copyBtn.className = "act-btn primary"; copyBtn.textContent = "复制提示词"; copyBtn.onclick = (e) => { e.stopPropagation(); copyText(d.prompt || ""); };
  ...
  return {
    ...
    getCatStyle, getBatch, imgUrl, getTags, applySort, toggle,
